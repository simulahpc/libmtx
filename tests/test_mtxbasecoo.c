...
/**
 * ‘test_mtxbasecoo_from_mtxfile()’ tests converting Matrix
 * Market files to matrices.
 */
int test_mtxbasecoo_from_mtxfile(void)
{
    int err;

    {
        ...
        struct mtxmatrix x;
        err = mtxmatrix_from_mtxfile(&x, mtxbasecoo, &mtxfile);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        TEST_ASSERT_EQ(mtxbasecoo, x.type);
        const struct mtxbasecoo * x_ = &x.storage.coo;
...
/**
 * ‘test_mtxbasecoo_to_mtxfile()’ tests converting matrices
 * to Matrix Market files.
 */
int test_mtxbasecoo_to_mtxfile(void)
{
    int err;

    {
        ...
        err = mtxmatrix_init_entries_real_single(
            &A, mtxbasecoo, mtx_unsymmetric, num_rows, num_columns, nnz, rowidx, colidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        struct mtxfile mtxfile;
...

    {
        ...
        struct mtxmatrix A;
        err = mtxmatrix_init_entries_real_single(
            &A, mtxbasecoo, mtx_symmetric, num_rows, num_columns, nnz, rowidx, colidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        struct mtxfile mtxfile;
        err = mtxmatrix_to_mtxfile(&mtxfile, &A, 0, NULL, 0, NULL, mtxfile_coordinate);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
...

    {
        ...
        struct mtxmatrix A;
        err = mtxmatrix_init_entries_real_single(
            &A, mtxbasecoo, mtx_skew_symmetric, num_rows, num_columns, nnz, rowidx, colidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        struct mtxfile mtxfile;
        err = mtxmatrix_to_mtxfile(&mtxfile, &A, 0, NULL, 0, NULL, mtxfile_coordinate);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
...

    {
        ...
        struct mtxmatrix A;
        err = mtxmatrix_init_entries_complex_single(
            &A, mtxbasecoo, mtx_hermitian, num_rows, num_columns, nnz, rowidx, colidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        struct mtxfile mtxfile;
        err = mtxmatrix_to_mtxfile(&mtxfile, &A, 0, NULL, 0, NULL, mtxfile_coordinate);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
...

    {
        ...
        err = mtxmatrix_init_entries_pattern(
            &A, mtxbasecoo, mtx_unsymmetric, num_rows, num_columns, nnz, rowidx, colidx);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        struct mtxfile mtxfile;
        err = mtxmatrix_to_mtxfile(&mtxfile, &A, 0, NULL, 0, NULL, mtxfile_coordinate);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
...
/**
 * ‘test_mtxbasecoo_partition_rowwise()’ tests partitioning
 * matrices rowwise.
 */
int test_mtxbasecoo_partition_rowwise(void)
{
    int err;
    {
        ...
        err = mtxmatrix_init_entries_real_single(
            &src, mtxbasecoo, mtx_unsymmetric,
...
/**
 * ‘test_mtxbasecoo_partition_columnwise()’ tests partitioning
 * matrices columnwise.
 */
int test_mtxbasecoo_partition_columnwise(void)
{
    int err;
    {
        struct mtxmatrix src;
...
/**
 * ‘test_mtxbasecoo_partition_2d()’ tests partitioning matrices in
 * a 2D manner.
 */
int test_mtxbasecoo_partition_2d(void)
{
    int err;
    {
        struct mtxmatrix src;
...
/**
 * ‘test_mtxbasecoo_split()’ tests splitting matrices.
 */
int test_mtxbasecoo_split(void)
{
    int err;
    {
        struct mtxmatrix src;
        ...
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        TEST_ASSERT_EQ(mtxbasecoo, dst0.type);
        TEST_ASSERT_EQ(mtx_unsymmetric, dst0.storage.coo.symmetry);
        TEST_ASSERT_EQ(3, dst0.storage.coo.num_rows);
        TEST_ASSERT_EQ(3, dst0.storage.coo.num_columns);
        TEST_ASSERT_EQ(4, dst0.storage.coo.num_nonzeros);
        TEST_ASSERT_EQ(4, dst0.storage.coo.size);
        TEST_ASSERT_EQ(dst0.storage.coo.rowidx[0], 0); TEST_ASSERT_EQ(dst0.storage.coo.colidx[0], 0);
        TEST_ASSERT_EQ(dst0.storage.coo.rowidx[1], 1); TEST_ASSERT_EQ(dst0.storage.coo.colidx[1], 0);
        TEST_ASSERT_EQ(dst0.storage.coo.rowidx[2], 1); TEST_ASSERT_EQ(dst0.storage.coo.colidx[2], 1);
        TEST_ASSERT_EQ(dst0.storage.coo.rowidx[3], 2); TEST_ASSERT_EQ(dst0.storage.coo.colidx[3], 1);
        TEST_ASSERT_EQ(mtx_field_real, dst0.storage.coo.a.field);
        TEST_ASSERT_EQ(mtx_single, dst0.storage.coo.a.precision);
        TEST_ASSERT_EQ(dst0.storage.coo.a.data.real_single[0], 1.0f);
        TEST_ASSERT_EQ(dst0.storage.coo.a.data.real_single[1], 4.0f);
        TEST_ASSERT_EQ(dst0.storage.coo.a.data.real_single[2], 5.0f);
        TEST_ASSERT_EQ(dst0.storage.coo.a.data.real_single[3], 8.0f);
        TEST_ASSERT_EQ(mtxbasecoo, dst1.type);
        TEST_ASSERT_EQ(mtx_unsymmetric, dst1.storage.coo.symmetry);
        TEST_ASSERT_EQ(3, dst1.storage.coo.num_rows);
        TEST_ASSERT_EQ(3, dst1.storage.coo.num_columns);
        TEST_ASSERT_EQ(4, dst1.storage.coo.num_nonzeros);
        TEST_ASSERT_EQ(4, dst1.storage.coo.size);
        TEST_ASSERT_EQ(dst1.storage.coo.rowidx[0], 0); TEST_ASSERT_EQ(dst1.storage.coo.colidx[0], 1);
        TEST_ASSERT_EQ(dst1.storage.coo.rowidx[1], 1); TEST_ASSERT_EQ(dst1.storage.coo.colidx[1], 2);
        TEST_ASSERT_EQ(dst1.storage.coo.rowidx[2], 2); TEST_ASSERT_EQ(dst1.storage.coo.colidx[2], 0);
        TEST_ASSERT_EQ(dst1.storage.coo.rowidx[3], 2); TEST_ASSERT_EQ(dst1.storage.coo.colidx[3], 2);
        TEST_ASSERT_EQ(mtx_field_real, dst1.storage.coo.a.field);
        TEST_ASSERT_EQ(mtx_single, dst1.storage.coo.a.precision);
        TEST_ASSERT_EQ(dst1.storage.coo.a.data.real_single[0], 2.0f);
        TEST_ASSERT_EQ(dst1.storage.coo.a.data.real_single[1], 6.0f);
        TEST_ASSERT_EQ(dst1.storage.coo.a.data.real_single[2], 7.0f);
        TEST_ASSERT_EQ(dst1.storage.coo.a.data.real_single[3], 9.0f);
...
/**
 * ‘test_mtxbasecoo_gemv()’ tests computing matrix-vector
 * products for matrices in coordinate format.
 */
int test_mtxbasecoo_gemv(void)
{
    int err;
...

    {
        ...
        err = mtxmatrix_init_entries_real_single(
            &A, mtxbasecoo, mtx_unsymmetric, num_rows, num_columns, num_nonzeros, Arowidx, Acolidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
...

    {
        ...
        float ydata[] = {1.0f, 0.0f, 2.0f};
        err = mtxmatrix_init_entries_real_single(
            &A, mtxbasecoo, mtx_symmetric, num_rows, num_columns, num_nonzeros, Arowidx, Acolidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        err = mtxvector_init_real_single(&x, mtxbasevector, num_columns, xdata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        {
...

    {
        ...
        float ydata[][2] = {{1.0f,0.0f}, {2.0f,2.0f}};
        err = mtxmatrix_init_entries_complex_single(
            &A, mtxbasecoo, mtx_hermitian, num_rows, num_columns, num_nonzeros, Arowidx, Acolidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        err = mtxvector_init_complex_single(&x, mtxbasevector, num_columns, xdata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        {
...
int main(int argc, char * argv[])
{
    ...
    TEST_RUN(test_mtxbasecoo_from_mtxfile);
    TEST_RUN(test_mtxbasecoo_to_mtxfile);
    TEST_RUN(test_mtxbasecoo_partition_rowwise);
    TEST_RUN(test_mtxbasecoo_partition_columnwise);
    TEST_RUN(test_mtxbasecoo_partition_2d);
    TEST_RUN(test_mtxbasecoo_split);
    TEST_RUN(test_mtxbasecoo_gemv);
    TEST_SUITE_END();
