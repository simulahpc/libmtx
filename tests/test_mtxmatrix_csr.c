...
int test_mtxmatrix_csr_to_mtxfile(void)
{
    ...
    }
    {
        ...
        struct mtxmatrix A;
        err = mtxmatrix_init_csr_real_single(
            &A, mtx_symmetric, num_rows, num_columns, rowptr, colidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        struct mtxfile mtxfile;
        err = mtxmatrix_to_mtxfile(&mtxfile, &A, 0, NULL, 0, NULL, mtxfile_coordinate);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
    ...
    }
    {
        ...
        int num_columns = 3;
        int nnz = 3;
        int64_t rowptr[] = {0, 2, 3, 3};
        int colidx[] = {2, 3, 3};
        float Adata[] = {2.0f, 3.0f, 6.0f};
        struct mtxmatrix A;
        err = mtxmatrix_init_csr_real_single(
            &A, mtx_skew_symmetric, num_rows, num_columns, rowptr, colidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        struct mtxfile mtxfile;
        err = mtxmatrix_to_mtxfile(&mtxfile, &A, 0, NULL, 0, NULL, mtxfile_coordinate);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
...

    {
        ...
        struct mtxmatrix A;
        err = mtxmatrix_init_csr_complex_single(
            &A, mtx_hermitian, num_rows, num_columns, rowptr, colidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        struct mtxfile mtxfile;
        err = mtxmatrix_to_mtxfile(&mtxfile, &A, 0, NULL, 0, NULL, mtxfile_coordinate);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
...
int test_mtxmatrix_csr_gemv(void)
{
     ...
     */
    {
        ...
        float ydata[] = {1.0f, 0.0f, 2.0f};
        err = mtxmatrix_init_csr_real_single(
            &A, mtx_symmetric, num_rows, num_columns, Arowptr, Acolidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        err = mtxvector_init_array_real_single(&x, num_columns, xdata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        {
...

    {
        ...
        float ydata[][2] = {{1.0f,0.0f}, {2.0f,2.0f}};
        err = mtxmatrix_init_csr_complex_single(
            &A, mtx_hermitian, num_rows, num_columns, Arowptr, Acolidx, Adata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        err = mtxvector_init_array_complex_single(&x, num_columns, xdata);
        TEST_ASSERT_EQ_MSG(MTX_SUCCESS, err, "%s", mtxstrerror(err));
        {
