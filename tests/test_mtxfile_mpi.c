...
int test_mtxfile_sendrecv(void)
{
    ...
    if (comm_size > 1) {
        if (rank == 0) {
            err = mtxfile_send(&srcmtx, 1, 0, comm, &disterr);
            if (err)
                mtxfile_free(&srcmtx);
            TEST_ASSERT_EQ(MTX_SUCCESS, err);
        } else if (rank == 1) {
            err = mtxfile_recv(&destmtx, 0, 0, comm, &disterr);
            TEST_ASSERT_EQ(MTX_SUCCESS, err);
        }
    } else {
        mtxfile_free(&srcmtx);
...
int test_mtxfile_bcast(void)
{
    ...
    err = mtxfile_bcast(&mtx, 0, comm, &disterr);
    if (err)
        mtxfile_free(&mtx);
    TEST_ASSERT_EQ(MTX_SUCCESS, err);
...
int test_mtxfile_gather(void)
{
...

    {
        ...
        err = mtxfile_gather(&srcmtx, dstmtxs, root, comm, &disterr);
        TEST_ASSERT_EQ_MSG(
            MTX_SUCCESS, err, "%s",
...
int test_mtxfile_allgather(void)
{
...

    {
        ...
        struct mtxfile dstmtxs[2];
        err = mtxfile_allgather(&srcmtx, dstmtxs, comm, &disterr);
        TEST_ASSERT_EQ_MSG(
            MTX_SUCCESS, err, "%s",
            err == MTX_ERR_MPI_COLLECTIVE
            ? mtxdisterror_description(&disterr)
...
int test_mtxfile_scatter(void)
{
...

    {
        ...
        err = mtxfile_scatter(srcmtxs, &dstmtx, root, comm, &disterr);
        TEST_ASSERT_EQ_MSG(
            MTX_SUCCESS, err, "%s",
            err == MTX_ERR_MPI_COLLECTIVE
            ? mtxdisterror_description(&disterr)
...
int test_mtxfile_alltoall(void)
{
...

    {
        ...
        struct mtxfile dstmtxs[2];
        err = mtxfile_alltoall(srcmtxs, dstmtxs, comm, &disterr);
        TEST_ASSERT_EQ_MSG(
            MTX_SUCCESS, err, "%s",
            err == MTX_ERR_MPI_COLLECTIVE
            ? mtxdisterror_description(&disterr)
