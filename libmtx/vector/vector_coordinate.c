...
int mtxvector_coordinate_alloc(
    ...
    int64_t size)
{
    vector->indices = malloc(size * sizeof(int));
    if (!vector->indices)
    ...
    const float * data)
{
    ...
    err = mtxvector_coordinate_alloc(
        vector, mtx_field_real, mtx_single, num_entries, size);
    if (err)
        return err;
    for (int64_t k = 0; k < size; k++) {
    ...
    const double * data)
{
    ...
    err = mtxvector_coordinate_alloc(
        vector, mtx_field_real, mtx_double, num_entries, size);
    if (err)
        return err;
    for (int64_t k = 0; k < size; k++) {
        vector->indices[k] = indices[k];
    ...
    const float (* data)[2])
{
    ...
    err = mtxvector_coordinate_alloc(
        vector, mtx_field_complex, mtx_single, num_entries, size);
    if (err)
        return err;
    for (int64_t k = 0; k < size; k++) {
        vector->indices[k] = indices[k];
    ...
    const double (* data)[2])
{
    ...
    err = mtxvector_coordinate_alloc(
        vector, mtx_field_complex, mtx_double, num_entries, size);
    if (err)
        return err;
    for (int64_t k = 0; k < size; k++) {
        vector->indices[k] = indices[k];
    ...
    const int32_t * data)
{
    ...
    err = mtxvector_coordinate_alloc(
        vector, mtx_field_integer, mtx_single, num_entries, size);
    if (err)
        return err;
    for (int64_t k = 0; k < size; k++) {
        vector->indices[k] = indices[k];
    ...
    const int64_t * data)
{
    ...
    err = mtxvector_coordinate_alloc(
        vector, mtx_field_integer, mtx_double, num_entries, size);
    if (err)
        return err;
    for (int64_t k = 0; k < size; k++) {
        vector->indices[k] = indices[k];
    ...
    const int * indices)
{
    ...
    err = mtxvector_coordinate_alloc(
        vector, mtx_field_pattern, mtx_single, num_entries, size);
    if (err)
        return err;
    for (int64_t k = 0; k < size; k++)
        vector->indices[k] = indices[k];
    ...
    const struct mtxfile * mtxfile)
{
    ...
    if (mtxfile->header.field == mtxfile_real) {
        err = mtxvector_coordinate_alloc(
            vector, mtx_field_real, mtxfile->precision, num_entries, size);
        if (err) {
            mtxfile_free(&copy);
            return err;
        }
        ...
        err = mtxvector_coordinate_alloc(
            vector, mtx_field_complex, mtxfile->precision, num_entries, size);
        if (err) {
            mtxfile_free(&copy);
            return err;
        }
        ...
        err = mtxvector_coordinate_alloc(
            vector, mtx_field_integer, mtxfile->precision, num_entries, size);
        if (err) {
            mtxfile_free(&copy);
            return err;
        }
    ...
    enum mtxfileformat mtxfmt)
{
    ...
    if (vector->field == mtx_field_real) {
        err = mtxfile_alloc_vector_coordinate(
            ...
            vector->num_entries, vector->size);
        if (err)
            return err;
        if (vector->precision == mtx_single) {
    ...
    const struct mtxpartition * part)
{
    ...
    for (int p = 0; p < num_parts; p++) {
        err = mtxvector_to_mtxfile(&srcmtxfiles[p], &srcs[p], 0, NULL, mtxfile_coordinate);
        if (err) {
            for (int q = p-1; q >= 0; q--)
...
{
    return x->num_entries == y->num_entries
        && x->num_nonzeros == y->num_nonzeros
        && x->size == y->size
        && memcmp(x->indices, y->indices, x->size*sizeof(*x->indices)) == 0;
}

static int mtxvector_coordinate_to_array(
    ...
    struct mtxvector_coordinate * y)
{
    ...
    if (err) return err;
    return mtxvector_array_swap(&xarray, &yarray);
}
    ...
    const struct mtxvector_coordinate * x)
{
    ...
    err = mtxvector_coordinate_to_array(&yarray, y);
    if (err) return err;
    return mtxvector_array_copy(&yarray, &xarray);
}
    ...
    int64_t * num_flops)
{
    ...
    int err = mtxvector_coordinate_to_array(&xarray, x);
    if (err) return err;
    return mtxvector_array_sscal(a, &xarray, num_flops);
}
    ...
    int64_t * num_flops)
{
    ...
    int err = mtxvector_coordinate_to_array(&xarray, x);
    if (err) return err;
    return mtxvector_array_dscal(a, &xarray, num_flops);
}
    ...
    int64_t * num_flops)
{
    ...
    int err = mtxvector_coordinate_to_array(&xarray, x);
    if (err) return err;
    return mtxvector_array_cscal(a, &xarray, num_flops);
}
    ...
    int64_t * num_flops)
{
    ...
    int err = mtxvector_coordinate_to_array(&xarray, x);
    if (err) return err;
    return mtxvector_array_zscal(a, &xarray, num_flops);
}
    ...
    int64_t * num_flops)
{
    if (y->type == mtxvector_array) {
        struct mtxvector xcoordinate;
        xcoordinate.type = mtxvector_coordinate;
        xcoordinate.storage.coordinate = *x;
        return mtxvector_array_sdot(&y->storage.array, &xcoordinate, dot, num_flops);
    } else if (y->type == mtxvector_coordinate) {
        const struct mtxvector_coordinate * ycoordinate = &y->storage.coordinate;
        if (x->field != ycoordinate->field) return MTX_ERR_INCOMPATIBLE_FIELD;
        ...
        int err = mtxvector_coordinate_to_array(&xarray, x);
        if (err) return err;
        struct mtxvector yarray;
        yarray.type = mtxvector_array;
        err = mtxvector_coordinate_to_array(&yarray.storage.array, ycoordinate);
        if (err) return err;
        return mtxvector_array_sdot(&xarray, &yarray, dot, num_flops);
    } else { return MTX_ERR_INVALID_VECTOR_TYPE; }
    return MTX_SUCCESS;
}
    ...
    int64_t * num_flops)
{
    if (y->type == mtxvector_array) {
        ...
        xcoordinate.type = mtxvector_coordinate;
        xcoordinate.storage.coordinate = *x;
        return mtxvector_array_ddot(&y->storage.array, &xcoordinate, dot, num_flops);
    } else if (y->type == mtxvector_coordinate) {
        const struct mtxvector_coordinate * ycoordinate = &y->storage.coordinate;
        if (x->field != ycoordinate->field) return MTX_ERR_INCOMPATIBLE_FIELD;
        if (x->precision != ycoordinate->precision) return MTX_ERR_INCOMPATIBLE_PRECISION;
        ...
        err = mtxvector_coordinate_to_array(&yarray.storage.array, ycoordinate);
        if (err) return err;
        return mtxvector_array_ddot(&xarray, &yarray, dot, num_flops);
    } else { return MTX_ERR_INVALID_VECTOR_TYPE; }
    return MTX_SUCCESS;
}
    ...
    int64_t * num_flops)
{
    if (y->type == mtxvector_array) {
        ...
        xcoordinate.type = mtxvector_coordinate;
        xcoordinate.storage.coordinate = *x;
        return mtxvector_array_cdotu(&y->storage.array, &xcoordinate, dot, num_flops);
    } else if (y->type == mtxvector_coordinate) {
        const struct mtxvector_coordinate * ycoordinate = &y->storage.coordinate;
        if (x->field != ycoordinate->field) return MTX_ERR_INCOMPATIBLE_FIELD;
        if (x->precision != ycoordinate->precision) return MTX_ERR_INCOMPATIBLE_PRECISION;
        ...
        err = mtxvector_coordinate_to_array(&yarray.storage.array, ycoordinate);
        if (err) return err;
        return mtxvector_array_cdotu(&xarray, &yarray, dot, num_flops);
    } else { return MTX_ERR_INVALID_VECTOR_TYPE; }
    return MTX_SUCCESS;
}
    ...
    int64_t * num_flops)
{
    if (y->type == mtxvector_array) {
        ...
        xcoordinate.type = mtxvector_coordinate;
        xcoordinate.storage.coordinate = *x;
        return mtxvector_array_zdotu(&y->storage.array, &xcoordinate, dot, num_flops);
    } else if (y->type == mtxvector_coordinate) {
        const struct mtxvector_coordinate * ycoordinate = &y->storage.coordinate;
        if (x->field != ycoordinate->field) return MTX_ERR_INCOMPATIBLE_FIELD;
        if (x->precision != ycoordinate->precision) return MTX_ERR_INCOMPATIBLE_PRECISION;
        ...
        err = mtxvector_coordinate_to_array(&yarray.storage.array, ycoordinate);
        if (err) return err;
        return mtxvector_array_zdotu(&xarray, &yarray, dot, num_flops);
    } else { return MTX_ERR_INVALID_VECTOR_TYPE; }
    return MTX_SUCCESS;
}
    ...
    int64_t * num_flops)
{
    if (y->type == mtxvector_array) {
        ...
        xcoordinate.type = mtxvector_coordinate;
        xcoordinate.storage.coordinate = *x;
        return mtxvector_array_cdotc(&y->storage.array, &xcoordinate, dot, num_flops);
    } else if (y->type == mtxvector_coordinate) {
        const struct mtxvector_coordinate * ycoordinate = &y->storage.coordinate;
        if (x->field != ycoordinate->field) return MTX_ERR_INCOMPATIBLE_FIELD;
        if (x->precision != ycoordinate->precision) return MTX_ERR_INCOMPATIBLE_PRECISION;
        ...
        err = mtxvector_coordinate_to_array(&yarray.storage.array, ycoordinate);
        if (err) return err;
        return mtxvector_array_cdotc(&xarray, &yarray, dot, num_flops);
    } else { return MTX_ERR_INVALID_VECTOR_TYPE; }
    return MTX_SUCCESS;
}
    ...
    int64_t * num_flops)
{
    if (y->type == mtxvector_array) {
        ...
        xcoordinate.type = mtxvector_coordinate;
        xcoordinate.storage.coordinate = *x;
        return mtxvector_array_zdotc(&y->storage.array, &xcoordinate, dot, num_flops);
    } else if (y->type == mtxvector_coordinate) {
        const struct mtxvector_coordinate * ycoordinate = &y->storage.coordinate;
        if (x->field != ycoordinate->field) return MTX_ERR_INCOMPATIBLE_FIELD;
        if (x->precision != ycoordinate->precision) return MTX_ERR_INCOMPATIBLE_PRECISION;
        ...
        err = mtxvector_coordinate_to_array(&yarray.storage.array, ycoordinate);
        if (err) return err;
        return mtxvector_array_zdotc(&xarray, &yarray, dot, num_flops);
    } else { return MTX_ERR_INVALID_VECTOR_TYPE; }
    return MTX_SUCCESS;
}
    ...
    int64_t * num_flops)
{
    ...
    int err = mtxvector_coordinate_to_array(&xarray, x);
    if (err) return err;
    return mtxvector_array_snrm2(&xarray, nrm2, num_flops);
}
    ...
    int64_t * num_flops)
{
    ...
    int err = mtxvector_coordinate_to_array(&xarray, x);
    if (err) return err;
    return mtxvector_array_dnrm2(&xarray, nrm2, num_flops);
}
    ...
    int64_t * num_flops)
{
    ...
    int err = mtxvector_coordinate_to_array(&xarray, x);
    if (err) return err;
    return mtxvector_array_sasum(&xarray, asum, num_flops);
}
    ...
    int64_t * num_flops)
{
    ...
    int err = mtxvector_coordinate_to_array(&xarray, x);
    if (err) return err;
    return mtxvector_array_dasum(&xarray, asum, num_flops);
}
