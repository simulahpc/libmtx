    ...
    struct program_options * args)
{
    ...
    if (args->permpath) free(args->permpath);
}
    ...
    int * nargs)
{
    ...
    *nargs = 0;
    ...
    while (*nargs < argc) {
        ...
        if (strstr(argv[0], "--sorting") == argv[0]) {
            int n = strlen("--sorting");
            const char * s = &argv[0][n];
            if (*s == '=') { s++; }
            else if (*s == '\0' && argc-*nargs > 1) { (*nargs)++; argv++; s=argv[0]; }
            else { program_options_free(args); return EINVAL; }
            err = parse_mtxfilesorting(&args->sorting, s, (char **) &s, NULL);
            if (err || *s != '\0') { program_options_free(args); return EINVAL; }
            (*nargs)++; argv++; continue;
        }

        if (strstr(argv[0], "--perm-path") == argv[0]) {
            int n = strlen("--perm-path");
            const char * s = &argv[0][n];
            if (*s == '=') { s++; }
            else if (*s == '\0' && argc-*nargs > 1) { (*nargs)++; argv++; s=argv[0]; }
            else { program_options_free(args); return EINVAL; }
            if (args->permpath) free(args->permpath);
            args->permpath = strdup(s);
            if (!args->permpath) { program_options_free(args); return errno; }
            (*nargs)++; argv++; continue;
        }
...
int main(int argc, char *argv[])
{
    ...
    if (args.sorting == mtxfile_permutation && !args.permpath) {
    ...
    if (!args.quiet) {
        ...
        err = mtxfilecomments_printf(
            ...
            program_name, program_version, mtxfilesortingstr(args.sorting));
        if (err) {
            if (args.verbose > 0)
                fprintf(diagf, "\n");
            fprintf(stderr, "%s: %s\n",
    ...
    if (args.sorting != mtxfile_permutation && args.permpath) {
        ...
        int64_t bytes_written = 0;
        err = mtxfile_write(
            &perm_mtxfile, args.permpath, false, NULL, &bytes_written);
        if (err) {
            if (args.verbose > 0)
                fprintf(diagf, "\n");
            fprintf(stderr, "%s: %s\n",
