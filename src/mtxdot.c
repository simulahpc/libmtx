    ...
    if (mtxdistfilex->header.field == mtxfile_complex) {
        if (precision == mtx_single) {
            ...
            for (int i = 0; i < repeat; i++) {
                if (verbose > 0) {
                    fprintf(diagf, "mtxmpivector_cdotc: ");
                    fflush(diagf);
                    clock_gettime(CLOCK_MONOTONIC, &t0);
                }
                int64_t num_flops = 0;
                err = mtxmpivector_cdotc(&x, &y, &dot, &num_flops, disterr);
                if (err) {
                    if (verbose > 0) fprintf(diagf, "\n");
                    mtxmpivector_free(&y);
                    mtxmpivector_free(&x);
            ...
            for (int i = 0; i < repeat; i++) {
                if (verbose > 0) {
                    fprintf(diagf, "mtxmpivector_zdotc: ");
                    fflush(diagf);
                    clock_gettime(CLOCK_MONOTONIC, &t0);
                }
                int64_t num_flops = 0;
                err = mtxmpivector_zdotc(&x, &y, &dot, &num_flops, disterr);
                if (err) {
                    if (verbose > 0) fprintf(diagf, "\n");
                    mtxmpivector_free(&y);
                    mtxmpivector_free(&x);
        ...
        if (precision == mtx_single) {
            ...
            for (int i = 0; i < repeat; i++) {
                if (verbose > 0) {
                    fprintf(diagf, "mtxmpivector_sdot: ");
                    fflush(diagf);
                    clock_gettime(CLOCK_MONOTONIC, &t0);
                }
                int64_t num_flops = 0;
                err = mtxmpivector_sdot(&x, &y, &dot, &num_flops, disterr);
                if (err) {
                    if (verbose > 0) fprintf(diagf, "\n");
                    mtxmpivector_free(&y);
                    mtxmpivector_free(&x);
            ...
            for (int i = 0; i < repeat; i++) {
                if (verbose > 0) {
                    fprintf(diagf, "mtxmpivector_ddot: ");
                    fflush(diagf);
                    clock_gettime(CLOCK_MONOTONIC, &t0);
                }
                int64_t num_flops = 0;
                err = mtxmpivector_ddot(&x, &y, &dot, &num_flops, disterr);
                if (err) {
                    if (verbose > 0) fprintf(diagf, "\n");
                    mtxmpivector_free(&y);
                    mtxmpivector_free(&x);
