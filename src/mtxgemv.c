    ...
    int * nargs)
{
    ...
    *nargs = 0;
    ...
    while (*nargs < argc) {
        ...
        if (strstr(argv[0], "--trans") == argv[0]) {
            int n = strlen("--trans");
            const char * s = &argv[0][n];
            if (*s == '=') { s++; }
            else if (*s == '\0' && argc-*nargs > 1) { (*nargs)++; argv++; s=argv[0]; }
            else { program_options_free(args); return EINVAL; }
            err = parse_mtxtransposition(&args->trans, s, (char **) &s, NULL);
            if (err || *s != '\0') { program_options_free(args); return EINVAL; }
            (*nargs)++; argv++; continue;
        }

        if (strstr(argv[0], "--overlap") == argv[0]) {
            int n = strlen("--overlap");
            const char * s = &argv[0][n];
            if (*s == '=') { s++; }
            else if (*s == '\0' && argc-*nargs > 1) { (*nargs)++; argv++; s=argv[0]; }
            else { program_options_free(args); return EINVAL; }
            err = parse_mtxgemvoverlap(&args->overlap, s, (char **) &s, NULL);
            if (err || *s != '\0') { program_options_free(args); return EINVAL; }
            (*nargs)++; argv++; continue;
        }
    ...
    struct mtxdisterror * disterr)
{
    ...
    if (verbose > 0) {
        fprintf(diagf, "mtxmpimatrix_from_mtxdistfile: ");
        fflush(diagf);
    ...
    if (mtxdistfilex) {
        ...
        enum mtxprecision precision;
        err = err ? err : mtxmpimatrix_precision(&A, &precision);
    ...
    if (mtxdistfiley) {
        ...
        err = mtxmpimatrix_alloc_column_vector(&A, &y, vectortype, disterr);
        if (err) {
            if (verbose > 0) fprintf(diagf, "\n");
            mtxmpimatrix_free(&A);
            return err;
    ...
    if (verbose > 0) {
        fprintf(diagf, "mtxmpimatrix_gemv_init: ");
        fflush(diagf);
        clock_gettime(CLOCK_MONOTONIC, &t0);
    }
    struct mtxmpimatrix_gemv gemv;
    err = mtxmpimatrix_gemv_init(
    ...
    if (precision == mtx_single) {
        for (int i = 0; i < repeat; i++) {
            MPI_Barrier(comm);
            if (verbose > 0) {
                fprintf(diagf, "mtxmpimatrix_gemv_sgemv: ");
                fflush(diagf);
                clock_gettime(CLOCK_MONOTONIC, &t0);
            }
            int64_t num_flops = 0;
            err = mtxmpimatrix_gemv_sgemv(&gemv, alpha, 1, disterr);
            if (err) {
                if (verbose > 0) fprintf(diagf, "\n");
        ...
        for (int i = 0; i < repeat; i++) {
            MPI_Barrier(comm);
            if (verbose > 0) {
                fprintf(diagf, "mtxmpimatrix_gemv_dgemv: ");
                fflush(diagf);
                clock_gettime(CLOCK_MONOTONIC, &t0);
            }
            int64_t num_flops = 0;
            err = mtxmpimatrix_gemv_dgemv(&gemv, alpha, 1, disterr);
            if (err) {
                if (verbose > 0) fprintf(diagf, "\n");
                mtxmpimatrix_gemv_free(&gemv);
                mtxmpivector_free(&y);
